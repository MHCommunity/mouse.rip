...
const updateUserScripts = async () => {
  ...
  for (const script of userscripts) {
    ...
    // Validate and ensure all numeric values are numbers above 0
    const validateNumber = (value, fallback = 0) => {
      const num = Number(value);
      return (!isNaN(num) && num > 0) ? num : Math.max(fallback, 0);
    };
    ...
    userscripts[scriptIndex] = {
      ...
      data: {
        ...
        installs: validateNumber(scriptData.total_installs),
        active_users: validateNumber(estimatedActiveUsers),
        installs_last_3_months: validateNumber(installsLast3Months),
        installs_last_month: validateNumber(installsLastMonth),
