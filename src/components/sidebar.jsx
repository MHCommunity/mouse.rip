...
export function SidebarSmallItem({ current, className, children, ...props }) {
  return (
    <SidebarItem
      {...props}
      current={current}
      className={clsx(className, 'text-xs/6 font-semibold text-zinc-500 dark:text-zinc-400')}
    >
      {children}
    </SidebarItem>
  );
...
export function SidebarLabel({ className, ...props }) {
  return <span {...props} className={clsx(className, 'truncate font-semibold')} />;
}
